...
// SPA history-mode prefixes. Any URL whose path starts with one of these
// is rewritten to / and re-served by the static file handler so the SPA
// router can take over. Keep this list aligned with the routes the
// frontend exposes; an unlisted prefix yields a hard 404 on reload.
var vueHistoryPaths = []string{
	// legacy Vue-frontend paths (kept for back-compat with older bundles)
	"dashboard",
	"auth",
	"inventory",
	"documentation",
	"settings",
	// shared between old and new bundles
	"tunnels",
	"commands",
	"scripts",
	// new ProxiPort SvelteKit SPA paths
	"login",
	"clients",
	"audit",
	"schedules",
