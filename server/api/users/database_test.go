...
func TestNewUserDatabase(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			_, err = NewUserDatabase(db, tc.UsersTable, tc.GroupsTable, tc.GroupDetailsTable, tc.twoFAOn, tc.totPOn, nil, testLog)
			if tc.ExpectedError == "" {
				require.NoError(t, err)
			} else {
				require.Error(t, err)
...
func TestListGroups(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			d, err := NewUserDatabase(db, "users", "groups", tc.DetailsTable, false, false, nil, testLog)
			require.NoError(t, err)
...
func TestUpdateGroup(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			d, err := NewUserDatabase(db, "users", "groups", "group_details", false, false, nil, testLog)
			require.NoError(t, err)
...
// TestUpdateGroupRoundtripsPermissionParams verifies that PermissionParams
// values written into a group survive a roundtrip through the database.
// The closed-source `plus/` package previously rejected malformed regex
// patterns, durations, and restriction rules at write time; that validation
// is gone in this OSS build, and the test cases that depended on it have
// been dropped. They will return when an OSS extended-permission engine is
// implemented.
func TestUpdateGroupRoundtripsPermissionParams(t *testing.T) {
	testCases := []struct {
		Name string
		Group
	}{
		{
			Name: "tunnels and commands",
