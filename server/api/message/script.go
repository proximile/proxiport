...
import (
	...
	email2 "github.com/proximile/proxiport/share/email"
)
...
func (s *ScriptService) DataToEnv(data Data) []string {
	return []string{
		fmt.Sprintf("PROXIPORT_2FA_TOKEN=%v", data.Token),
		fmt.Sprintf("PROXIPORT_2FA_SENDTO=%v", data.SendTo),
		fmt.Sprintf("PROXIPORT_2FA_TOKEN_TTL=%v", data.TTL.Seconds()),
		fmt.Sprintf("PROXIPORT_2FA_USER_AGENT=%v", data.UserAgent),
		fmt.Sprintf("PROXIPORT_2FA_REMOTE_ADDRESS=%v", data.RemoteAddress),
	}
}
