...
// go test -count=1 -race -v github.com/proximile/proxiport/server/clients -run TestStartClientConcurrency
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			internalTunnelProxyConfig := &clienttunnel.InternalTunnelProxyConfig{
				Enabled:  true,
				CertFile: "../../../testdata/certs/proxiport.test.crt",
				KeyFile:  "../../../testdata/certs/proxiport.test.key",
			}
			...
			requestedRemote.TunnelURL = "https://12345678.tunnels.proxiport.test"
			...
			assert.Equal(t, "tunnels.proxiport.test", mockCaddyAPI.NewRouteRequest.DownstreamProxyBaseDomain)
