...
import (
	...
	"github.com/proximile/proxiport/server/acme"
	apiErrors "github.com/proximile/proxiport/server/api/errors"
	"github.com/proximile/proxiport/server/caddy"
...
// UpdateClientStatus is retained for interface compatibility. Per-client
// pause/unpause based on a license seat count was a Plus-only feature and is
// not enforced in OSS builds.
...
func (s *ClientServiceProvider) StartTunnel(
	...
	if remote.AutoClose > 0 {
		...
		ctx, cancel = context.WithTimeout(ctx, remote.AutoClose)
		// The tunnel's lifetime is bounded by ctx; when ctx fires (timeout
		// or parent cancellation) we release the cancel func to satisfy
		// `go vet` without shortening the tunnel's lifetime.
		go func() {
			<-ctx.Done()
			cancel()
		}()
