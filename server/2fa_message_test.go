...
func TestScriptMessage(t *testing.T) {
	...
	tfaScript := struct {
		...
		name:    tmpDir + "tfa.sh",
		log:     tmpDir + "tfa.log",
		content: []byte("#!/bin/sh\nset|grep PROXIPORT_>" + tmpDir + "tfa.log"),
		mode:    0700,
	}
	...
	require.NoError(t, err)
	assert.Regexp(t, regexp.MustCompile(fmt.Sprintf("PROXIPORT_2FA_SENDTO='*%s'*", "t@example.com")), scriptLog)
	assert.Regexp(t, regexp.MustCompile("PROXIPORT_2FA_REMOTE_ADDRESS='*127.0.0.1'*"), scriptLog)
	assert.Regexp(t, regexp.MustCompile("PROXIPORT_2FA_USER_AGENT='*Go'*"), scriptLog)
	assert.Regexp(t, regexp.MustCompile("PROXIPORT_2FA_TOKEN='*[a-z A-Z 0-9]{6}'*"), scriptLog)
	assert.Regexp(t, regexp.MustCompile(fmt.Sprintf("PROXIPORT_2FA_TOKEN_TTL='*%d'*", tokenTTL)), scriptLog)
...
func TestPushoverMessage(t *testing.T) {
	...
	gock.Observe(func(request *http.Request, mock gock.Mock) {
		...
		assert.Regexp(t, regexp.MustCompile("Your ProxiPort 2fa token:<b>[a-z A-Z 0-9]{6}</b>"), body)
	})
