...
func FsCallback(fs *test.FileAPIMock, t *testing.T) {
	...
	fs.On("CreateFile", "/data/filepush/id-123_proxiport_filepush", mock.MatchedBy(fileExpectation)).Return(int64(10), nil)

	fileMock := &test.ReadWriteCloserMock{}
	fileMock.Reader = strings.NewReader("some content")
	fileMock.On("Close").Return(nil)

	fs.On("Open", "/data/filepush/id-123_proxiport_filepush").Return(fileMock, nil)
	fs.On("Remove", "/data/filepush/id-123_proxiport_filepush").Return(nil)
}
...
func TestHandleFileUploads(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			wantClientInputFile: &models.UploadedFile{
				ID:                   "id-123",
				SourceFilePath:       "/data/filepush/id-123_proxiport_filepush",
