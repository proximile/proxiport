...
import (
	...
	"github.com/proximile/proxiport/server/notifications/channels/toLog"
	...
	"github.com/proximile/proxiport/server/api/command"
	...
	"github.com/proximile/proxiport/server/vault"
...
type UserService interface {
	...
	GetEffectiveUserExtendedPermissions(*users.User) ([]users.PermissionParams, []users.PermissionParams)
}
...
func NewAPIListener(
	...
	vaultDBProviderFactory := vault.NewStatefulDbProviderFactory(
		func() (vault.DbProvider, error) {
			return vault.NewSqliteProvider(config, config.KeyProvider.Envelope(), vaultLogger)
		},
...
func (al *APIListener) shouldCreateMissingUser(user *users.User, skipPasswordValidation bool) bool {
	...
	return al.config.API.CreateMissingUsers
}
