...
type Server struct {
	...
	// for forwarding caddy logs into the proxiportd log
	logLogger *logger.Logger

	// readers below used in a pipe that we use for relaying caddy log messages into the proxiport log
	w *io.PipeWriter // where caddy should write log messages
	r *io.PipeReader // where proxiport should read caddy log messages
}
...
var caddyToProxiportLogLevelMap = map[string]string{
	"debug": "debug",
	"info":  "info",
	"warn":  "info",
	"error": "error",
...
func (c *Server) writeCaddyLogLine(line string) {
	...
	mapsTo, ok := caddyToProxiportLogLevelMap[level]
	if ok {
		switch mapsTo {
		case "debug":
