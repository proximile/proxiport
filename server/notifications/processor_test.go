...
func (suite *ProcessorTestSuite) TestProcessNotificationStateError() {
	// Set the failure flag BEFORE queueing so the processor goroutine
	// reads `fail == true` when it picks up the notification. The
	// inherited form set the flag after SendMail() and raced with the
	// processor; under a faster runtime the notification could complete
	// (state=Done) before the test's Store landed.
