...
import (
	...
	"github.com/proximile/proxiport/cmd/proxiportd/servicemanagement"
	...
	chserver "github.com/proximile/proxiport/server"
...
var serverHelp = `
  Usage: proxiportd [options]

  Examples:

    ./proxiportd --addr=0.0.0.0:9999 --auth=clientAuth1:1234
    starts server, listening to client connections on port 9999

    ./proxiportd --addr="[2a01:4f9:c010:b278::1]:9999" --api-addr=0.0.0.0:9000 --api-auth=admin:1234 --auth=clientAuth1:1234
    starts server, listening to client connections on IPv6 interface,
    also enabling HTTP API, available at http://0.0.0.0:9000/

    ./proxiportd -c /etc/proxiport/proxiportd.conf
  ...
  Options:
    ...
    This is where the proxiport clients connect to.
    ...
    This is for authentication of the proxiport tunnel clients.
    ...
    Use either "authfile", "auth-table" or "auth". If multiple auth options are enabled, proxiportd exits with an error.
    ...
    proxiportd receives a normal HTTP request. Useful for hiding proxiportd in
    plain sight.
    ...
    --api-doc-root, Specifies local directory path. If specified, proxiportd will serve
    ...
    By default, "/var/lib/proxiportd" is used on Linux, 'C:\ProgramData\proxiportd' is used on Windows.
    ...
    Ideally this directory is the homedir of the proxiport user and has been created along with the user.
    Example: useradd -r -d /var/lib/proxiportd -m -s /bin/false -U -c "System user for proxiport client and server" proxiport
    ...
    --max-request-bytes-client, An optional arg to define a limit for data that can be sent by proxiport clients.
    By default is set to 524288(512Kb).

    --allow-root, An optional arg to allow running proxiportd as root. There is no technical requirement to run the proxiport
    server under the root user. Running it as root is an unnecessary security risk.

    --service, Manages proxiportd running as a service. Possible commands are "install", "uninstall", "start" and "stop".
    The only arguments compatible with --service are --service-user and --config, others will be ignored.

    --service-user, An optional arg specifying user to run proxiportd service under. Only on linux. Defaults to proxiport.
    ...
    Config file should be in TOML format. You can find an example "proxiportd.example.conf" in the release archive.
    ...
    --novnc-root, Specifies local directory path. If specified, proxiportd will serve
    novnc javascript app from this directory.

    --guacd-address, Specifies network address (host:port) of guacd daemon. If specified, proxiportd will serve
  ...
  Signals:
    The proxiportd process is listening for SIGUSR2 to print process stats
...
func init() {
	...
	if runtime.GOOS != "windows" {
		svcUser = lFlags.String("service-user", "proxiport", "")
	}
	...
	viperCfg.SetDefault("api.totp_account_name", "ProxiPort")
	...
	viperCfg.SetDefault("notifications.notification_script_dir", "/usr/local/lib/proxiport/notification_scripts")
}
...
func runMain(*cobra.Command, []string) {
	...
	if writeCPUProfile {
		...
		cpuprof, err := os.Create("/var/lib/proxiport/cpu.proxiportd.prof")
		if err != nil {
			log.Fatal(err)
		}
		...
		defer func() {
			fmt.Println("writing cpu.proxiportd.prof")
...
func WriteMemoryProfile(l *logger.Logger) {
	l.Debugf("writing mem.proxiportd.prof")
