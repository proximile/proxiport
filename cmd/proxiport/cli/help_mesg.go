...
var ClientHelp = `
  ...
  <server>, proxiportd server address. Mandatory IP address and port divided by a colon, unless --config(-c) is used.
  ...
  Options:
    ...
    export the fingerprint to the environment variable PROXIPORT_FINGERPRINT.

    --auth, Required client authentication credentials in the form: "<client-auth-id>:<password>".
    Alternatively, export credentials to the environment variable PROXIPORT_AUTH.
    ...
    used to reach the proxiport server. Authentication can be specified
    inside the URL.
    ...
    --use-system-id, By default proxiport reads /etc/machine-id (Linux) or the ComputerSystemProduct UUID (Windows)
    ...
    --use-hostname, By default proxiport reads the local hostname to identify the system in a human-readable way.
    ...
    --allow-root, An optional arg to allow running proxiport as root. There is no technical requirement to run the proxiport
    client under the root user. Running it as root is an unnecessary security risk.

    --service, Manages proxiport running as a service. Possible commands are "install", "uninstall", "start" and "stop".
    ...
    Defaults: /var/lib/proxiport (unix) or C:\Program Files\proxiport (windows)
    ...
    --updates-interval, How often after the proxiport client has started pending updates are summarized.
    Defaults: 4h
   ...
   --monitoring-net-wan, enable monitoring of wan network card
    ...
    Note: --scheme refers to the local protocol. The proxiport server will always use https for the proxy.
   ...
   Environment Variables:
    PROXIPORT_AUTH
    PROXIPORT_FINGERPRINT

  Signals:
    The proxiport process is listening for:
